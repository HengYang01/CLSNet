...
class Block(nn.Module):
    def __init__(self,dim=150, stage=3):
        ...
        self.HIGF_1 = HIGF(dim_num=dim, heads=30)
        self.HIGF_2 = HIGF(dim_num=dim, heads=30)
        self.HIGF_3 = HIGF(dim_num=dim, heads=30)
        ...
        self.HIGF_4 = HIGF(dim_num=dim, heads=30)
        ...
        self.HIGF_5 = HIGF(dim_num=dim, heads=30)
        self.HIGF_6 = HIGF(dim_num=dim, heads=30)
        self.HIGF_7 = HIGF(dim_num=dim, heads=30)
...
class CLSNet(nn.Module):
    def __init__(self, in_channels=4, out_channels=150, n_feat=180, stage=3):
...
a = torch.rand(1, 150, 32, 32).cuda()
b = torch.rand(1, 4, 64, 64).cuda()
model = CLSNet().cuda()
print(model)
