        ...
        if epoch==1 or epoch % args.save_freq == 0 or args.epochs - epoch < 5:
            ...
            with torch.no_grad():
                for cnt, (lrhs, hrms, hrhs) in enumerate(val_dataloader, 1):
                    ...
                    val_loss = loss_func(prediction, hrhs_for_Indicator)#C,H,W计算指标
...
def main(args):
    ...
    maxiteration = math.ceil(((512 - args.train_size) // args.stride + 1) * ((512 - args.train_size) // args.stride + 1) * args.nums / args.batch_size) * args.epochs
