...
def train(fusion_net: nn.Module, optimizers,scheduler, train_dataloader, val_dataloader, args):
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        # val
        if epoch==1 or epoch % args.save_freq == 0 or args.epochs - epoch < 5:
            psnr_avg = 0.
            sam_avg = 0.
            rmse_avg = 0.
            val_avg_loss = 0.
            ...
            with torch.no_grad():
                for cnt, (lrhs, hrms, hrhs) in enumerate(val_dataloader, 1):
                    ...
                    prediction = (reconstruction(fusion_net, lrhs.cuda(), hrms.cuda(), downsample_factor = args.spatial_ratio, training_size = args.train_size, stride = (args.stride * args.spatial_ratio * args.spatial_ratio), batch_size=args.batch_size).cuda()).detach()
...
def main(args):
    ...
    maxiteration = math.ceil(((550 - args.train_size) // args.stride + 1) * ((572 - args.train_size) // args.stride + 1) * args.nums / args.batch_size) * args.epochs
    print("maxiteration:", maxiteration)


    train_set = MakeSimulateDataset_GF5(args, "train")
    train_dataloader = DataLoader(dataset=train_set, batch_size = args.batch_size, shuffle=True)
    val_set = MakeSimulateDataset_GF5(args, "test")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--stride', type=int, default=8, help='Stride when crop an original image into patches.')
    parser.add_argument('--batch_size', type=int, default=32, help='Batch size for training dataset.')
    parser.add_argument('--epochs', type=int, default=200, help='Total epochs to train the model.')
    ...
    parser.add_argument('--nums', type=int, default="1", help='Number of the images.')
