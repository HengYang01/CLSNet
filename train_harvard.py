        ...
        if epoch==1 or epoch % args.save_freq == 0 or args.epochs - epoch < 5:
            ...
            with torch.no_grad():
                for cnt, (lrhs, hrms, hrhs) in enumerate(val_dataloader, 1):
                    ...
                    prediction = (reconstruction(fusion_net, lrhs.cuda(), hrms.cuda(), downsample_factor = args.spatial_ratio, training_size = args.train_size, stride = args.stride, batch_size=args.batch_size).cuda()).detach()
...
def main(args):
    ...
    maxiteration = math.ceil(((1040 - args.train_size) // args.stride + 1) * ((1392 - args.train_size) // args.stride + 1) * args.nums / args.batch_size) * args.epochs
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default="HARVARD", help='Dataset to be loaded.')
    ...
    parser.add_argument('--nums', type=int, default="30", help='Number of the images.')
